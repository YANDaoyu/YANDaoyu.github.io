...
Use mathematics to solve this kind of problems!      
First, find `biggest` and `smallest` integer of the array, then minus operation level by level.     
The detail of minus is: find the `head` and `tail` of the array that no bigger than `someinteger`, and then `trapwater -= (head+tail)`.
